...
def _latest_run_per_experiment(runs_dir: Path = RUNS_DIR) -> dict[str, dict]:
...
def build_report(runs_dir: Path = RUNS_DIR) -> str:
    """Markdown table of the latest run per experiment, sorted by nDCG@10 descending."""
    runs = list(_latest_run_per_experiment(runs_dir).values())
...
def write_report(runs_dir: Path = RUNS_DIR, results_dir: Path = RESULTS_DIR) -> Path:
    results_dir.mkdir(parents=True, exist_ok=True)
    path = results_dir / "results.md"
    path.write_text(build_report(runs_dir), encoding="utf-8")
    return path
