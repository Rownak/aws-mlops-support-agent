"""Raw YAML loading with ``${VAR}`` environment substitution.

``Config`` is the untyped layer: dot-notation access to whatever is in the
file, including settings the typed ``RagConfig`` tree knows nothing about
(a source's ``aws_access_key_id``, say). ``RagConfig`` is built on top of it.
"""
