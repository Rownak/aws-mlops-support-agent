"""
Everything that writes to the vector store.

`Ingestor` owns the three ingest verbs (``ingest_documents``,
``ingest_directory``, ``sync``) and the prepare -> write path underneath
them. `RagCore` holds one and delegates, so ``rag.sync()`` and friends keep
working unchanged; this module is where the work actually happens.

Separated from the query side because the two have almost nothing in
common but the store: ingestion is about document identity, deduplication,
partial-write recovery and replacement of stale versions, none of which a
read-only query path ever touches. Keeping them apart means the rules that
decide *what counts as the same document* have one home — the thing that
changes when the chunk-ID policy changes.

All three entry points share one path — `_prepare_document` (pure, never
writes) feeding `_write_prepared` (the only mutator) — so partial-write
recovery and stale-version replacement behave identically however you
ingest.
"""
...
from .stats import IngestStats, empty_stats
...
class Ingestor:
    """
    Writes documents into the vector store, idempotently.

    Args:
        config: The loaded `RagConfig` — read for `splitter`, `vectorstore`,
            `sources` and `loader_extensions`
        store: The vector store wrapper (a `PineconeStore`), which owns
    ...
    def ingest_documents(
        ...
        *,
        ...
        Example:
            >>> stats = ingestor.ingest_documents(["a.pdf", "b.md"])  # doctest: +SKIP
            >>> print(f"Processed {stats['processed']} of {stats['total']}")
        """
        stats = empty_stats(len(file_paths))
        if not file_paths:
            return stats
    ...
    def ingest_directory(
        ...
        Example:
            >>> stats = ingestor.ingest_directory("data/", recursive=True)  # doctest: +SKIP
        """
