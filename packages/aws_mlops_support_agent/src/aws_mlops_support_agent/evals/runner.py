...
- **Escalation accuracy**: does the caller's REAL routing decision send each
  ...
  this runner tests the real code path without importing the agent graph.

Each row also reports `top_score` and `score_gap`, so "does score_gap add
signal?" is answerable by eye from the saved table.

This module and the dataset both live with THIS project (the questions and
the router are specific to this agent's retrieve → confidence → escalate
flow); only the underlying `rag_core.retriever.confidence` machinery is
generic.
"""
